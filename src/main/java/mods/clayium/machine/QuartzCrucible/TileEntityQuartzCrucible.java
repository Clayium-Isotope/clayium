...
public class TileEntityQuartzCrucible extends TileEntityGeneric implements ITickable {
    ...
    public boolean putIngot() {
        if (!this.world.isRemote && this.world.getBlockState(this.pos).getValue(BlockStateQuartzCrucible.LEVEL) < 9) {
            this.world.setBlockState(this.pos,
                    this.world.getBlockState(this.pos).cycleProperty(BlockStateQuartzCrucible.LEVEL), 3);
            return true;
        } else {
            return false;
        }
    ...
    public boolean consumeString() {
        ...
        if (this.world != null && !this.world.isRemote && this.craftTime >= ingotQuantity * timeToCraft) {
            EntityItem entityitem = new EntityItem(this.world, (float) this.getPos().getX() + 0.4F,
                    (float) this.getPos().getY() + 0.4F, (float) this.getPos().getZ() + 0.4F,
                    ClayiumMaterials.get(ClayiumMaterial.silicon, ClayiumShape.ingot, ingotQuantity));
            this.world.spawnEntity(entityitem);
            this.craftTime = 0;
            this.world.setBlockState(this.pos,
                    this.world.getBlockState(this.pos).withProperty(BlockStateQuartzCrucible.LEVEL, 0), 3);
            return true;
        } else {
            return false;
        }
