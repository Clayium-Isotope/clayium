...
public class ContainerVacuumContainer extends ContainerTemp {
    ...
    public ContainerVacuumContainer(InventoryPlayer player, TileEntityVacuumContainer tileEntity) {
        ...
        if (!tileEntity.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null)) {
            throw new IllegalArgumentException(
                    "Who removed CapabilityItemHandler.ITEM_HANDLER_CAPABILITY from TileEntityVacuumContainer?");
        }
    ...
    public void setMachineInventorySlots(InventoryPlayer player) {
        this.addMachineSlotToContainer(new SlotItemHandler((TileEntityVacuumContainer) this.tileEntity, 0,
                (this.machineGuiSizeX - 16) / 2, 35, RectangleTexture.LargeSlotTexture));
        this.addMachineSlotToContainer(
                new SlotMemory(this.tileEntity, 1, 108 + (this.machineGuiSizeX - 108) / 2, 18, true));
    }
