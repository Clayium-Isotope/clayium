...
public class EntityTeleportBall extends EntityClayBall {
    ...
    public EntityTeleportBall(World world, EntityLivingBase thrower, int lifespan, float initialVelocity,
                              float diffusion, int damage, int numberOfTick, boolean critical) {
        ...
        if (!this.world.isRemote) {
            if (this.getThrower() != null && this.getThrower() instanceof EntityPlayerMP) {
                ...
                if (entityplayermp.connection.getNetworkManager().isChannelOpen() &&
                        entityplayermp.world == this.world) {
                    ...
                    if (!MinecraftForge.EVENT_BUS.post(event)) {
                        ...
                        } while ((this.isCollidable(this.world, x, y, z) ||
                                this.isCollidable(this.world, x, y + 1.0D, z)) && c++ < 100);
                        ...
                        for (length = 0; length < 30; ++length) {
                            UtilNetwork.sendParticlePacketFromServer(server, EnumParticleTypes.PORTAL,
                                    event.getTargetX(), event.getTargetY() + this.rand.nextDouble() * 2.0D,
                                    event.getTargetZ(), 0, this.rand.nextGaussian(), 0.0D, this.rand.nextGaussian(),
                                    1.0D, 20.0D);
                        }

                        length = (int) Math.sqrt((event.getTargetX() - this.getThrower().posX) *
                                (event.getTargetX() - this.getThrower().posX) +
                                (event.getTargetY() - this.getThrower().posY) *
                                        (event.getTargetY() - this.getThrower().posY) +
                                (event.getTargetZ() - this.getThrower().posZ) *
                                        (event.getTargetZ() - this.getThrower().posZ));

                        for (int i = 0; i < length; ++i) {
                            double r = (double) i / (double) length;
                            UtilNetwork.sendParticlePacketFromServer(server, EnumParticleTypes.PORTAL,
                                    (event.getTargetX() - oldX) * r + oldX,
                                    (event.getTargetY() - oldY) * r + oldY + 1.0D,
                                    (event.getTargetZ() - oldZ) * r + oldZ, 0, (oldX - this.posX) / (double) length,
                                    (oldY - this.posY) / (double) length, (oldZ - this.posZ) / (double) length, 1.0D,
                                    200.0D);
                        }
    ...
    protected void spawnFlyingDustPartcle() {
        this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, this.lastTickPosX, this.lastTickPosY, this.lastTickPosZ,
                ...
                Block.getStateId(Blocks.WOOL.getDefaultState()) + 13);
        this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, (this.posX + this.lastTickPosX) / 2.0D,
                ...
                this.motionY / 2.0D, this.motionZ / 2.0D, Block.getStateId(Blocks.WOOL.getDefaultState()) + 13);
    }
    ...
    protected void playImpactSound() {
        this.playSound(SoundEvents.BLOCK_STONE_STEP, 0.06F * (float) this.damage,
