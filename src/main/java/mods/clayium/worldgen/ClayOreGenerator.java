...
public class ClayOreGenerator implements IWorldGenerator {
    ...
    private void generateOre(World world, Random random, ChunkPos chunk) {
        for (int i = 0; i < clayOreVeinNumber; ++i) {
            ...
            new WorldGenMinable(ClayiumBlocks.clayOre.getDefaultState(), clayOreVeinSize, p -> p == Blocks.STONE.getDefaultState())
                    .generate(world, random, pos);

            if (generateDenseClayOreVein) {
                new WorldGenMinable(ClayiumBlocks.denseClayOre.getDefaultState(), denseClayOreVeinSize, p -> p == ClayiumBlocks.clayOre.getDefaultState())
                        .generate(world, random, pos);
        ...
        for (int i = 0; i < largeDenseClayOreVeinNumber; ++i) {
            ...
            new WorldGenMinable(ClayiumBlocks.largeDenseClayOre.getDefaultState(), largeDenseClayOreVeinSize, p -> p == Blocks.STONE.getDefaultState())
                    .generate(world, random, pos);
        }
    }
}
