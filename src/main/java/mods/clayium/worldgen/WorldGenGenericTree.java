...
public class WorldGenGenericTree extends WorldGenAbstractTree {
    ...
    public WorldGenGenericTree(boolean notify, int minTreeHeight, IBlockState log, IBlockState leaves,
                               boolean vinesGrow) {
        super(notify);
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        if (!flag) {
            ...
            boolean isSoil = state.getBlock().canSustainPlant(state, worldIn, down, net.minecraft.util.EnumFacing.UP,
                    ...
                    if (state2.getBlock().isAir(state2, worldIn, upN) ||
                        ...
                        if (this.vinesGrow && j2 > 0) {
                            if (rand.nextInt(3) > 0 && worldIn.isAirBlock(upN.west())) {
                                this.setBlockAndNotifyAdequately(worldIn, upN.west(),
                                        Blocks.VINE.getDefaultState().withProperty(BlockVine.EAST, true));
                            }

                            if (rand.nextInt(3) > 0 && worldIn.isAirBlock(upN.east())) {
                                this.setBlockAndNotifyAdequately(worldIn, upN.east(),
                                        Blocks.VINE.getDefaultState().withProperty(BlockVine.WEST, true));
                            }

                            if (rand.nextInt(3) > 0 && worldIn.isAirBlock(upN.north())) {
                                this.setBlockAndNotifyAdequately(worldIn, upN.north(),
                                        Blocks.VINE.getDefaultState().withProperty(BlockVine.SOUTH, true));
                            }

                            if (rand.nextInt(3) > 0 && worldIn.isAirBlock(upN.south())) {
                                this.setBlockAndNotifyAdequately(worldIn, upN.south(),
                                        Blocks.VINE.getDefaultState().withProperty(BlockVine.NORTH, true));
                            }
                        }
                    }
                }
                    ...
                    for (int y = position.getY() - 3 + i; y <= position.getY() + i; ++y) {
                        ...
                        for (int x = position.getX() - l1; x <= position.getX() + l1; ++x) {
                            for (int z = position.getZ() - l1; z <= position.getZ() + l1; ++z) {
                                ...
                                if (worldIn.getBlockState(pos).getBlock().isLeaves(worldIn.getBlockState(pos), worldIn,
                                        pos)) {
                            ...
                            for (EnumFacing i3 : EnumFacing.HORIZONTALS) {
                                if (rand.nextInt(4 - k1) == 0) {
                                    this.setBlockAndNotifyAdequately(worldIn,
                                            new BlockPos(position.getX() + i3.getXOffset(),
                                                    position.getY() + i - 5 + k1, position.getZ() + i3.getZOffset()),
                                            Blocks.COCOA.getDefaultState()
                                                    .withProperty(BlockCocoa.FACING, i3.getOpposite()).withProperty(
                                                            BlockCocoa.AGE, rand.nextInt(3) << 2 | i3.getIndex()));
                                }
                            }
                        }
                    }
    ...
    private void growVines(World world, int x, int startY, int z, EnumFacing direction) {
        ...
        for (BlockPos pos1 : BlockPos.getAllInBox(x, startY, z, x, startY - i1, z)) {
            ...
            this.setBlockAndNotifyAdequately(world, pos1,
                    Blocks.VINE.getDefaultState().withProperty(BlockVine.getPropertyFor(direction), true));
        }
    }
}
