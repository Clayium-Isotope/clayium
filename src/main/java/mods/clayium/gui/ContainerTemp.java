...
public abstract class ContainerTemp extends Container {
    ...
    protected void setupPlayerSlots(InventoryPlayer player, int customYOffset) {
        for(int y = 0; y < 3; ++y) {
            for(int x = 0; x < 9; ++x) {
                this.addSlotToContainer(new Slot(player, x + y * 9 + 9, this.playerSlotOffsetX + 8 + x * 18, customYOffset + 12 + y * 18));
            }
        }
    ...
    public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
                ...
                if (!this.dragSlots.isEmpty())
                {
                    ...
                    for (Slot slot8 : this.dragSlots)
                    {
                        ...
                        if (slot8 != null && canAddItemToSlot(slot8, itemstack13, true) && slot8.isItemValid(itemstack13) && (/*this.dragMode == 2 ||*/itemstack13.getCount() >= this.dragSlots.size()) && this.canDragIntoSlot(slot8))
                        {
                            ...
                            if (!(slot8 instanceof mods.clayium.gui.SlotMemory))
        ...
        else if ((clickTypeIn == ClickType.PICKUP || clickTypeIn == ClickType.QUICK_MOVE) && (dragType == 0 || dragType == 1))
        {
            ...
            else
            {
                ...
                if (slot6 != null)
                {
                    ...
                    if (slot6 instanceof mods.clayium.gui.SlotMemory && !itemstack11.isEmpty())
                        itemstack11 = itemstack11.copy();
                    ...
                    if (itemstack8.isEmpty())
                    {
                        if (!itemstack11.isEmpty() && slot6.isItemValid(itemstack11))
                        {
                            ...
                            if (itemstack11.getCount() >= i3) {
                                slot6.putStack(itemstack11.splitStack(i3));
                            }

                            if (itemstack11.getCount() <= 0) {
                                if (!(slot6 instanceof mods.clayium.gui.SlotMemory)) {
                                    inventoryplayer.setItemStack(ItemStack.EMPTY);
                                }
                            }
                        }
        ...
        else if (clickTypeIn == ClickType.SWAP && dragType >= 0 && dragType < 9)
        {
            ...
            if (slot4.canTakeStack(player))
            {
                ...
                if (!itemstack3.isEmpty() && !(slot4.inventory == inventoryplayer && slot4.isItemValid(itemstack3)))
                {
                    l1 = inventoryplayer.getFirstEmptyStack();
                    i = l1 > -1 ? 1 : 0;
                }

                if (slot4.getHasStack() && i != 0)
                {
                    ...
                    if ((slot4.inventory != inventoryplayer || !slot4.isItemValid(itemstack3)) && !itemstack3.isEmpty())
                    {
                        if (l1 > -1)
                        {
                            ...
                            slot4.decrStackSize(itemstack5.getCount());
                            slot4.putStack(ItemStack.EMPTY);
                            slot4.onTake(player, itemstack5);
                        }
                    }
                    else
                    {
