...
public class DirectionMemory extends ItemTiered {
    ...
    public boolean onItemUseFirst(ItemStack itemstack, EntityPlayer player, World world, int x, int y, int z, int side, float posX, float posY, float posZ) {
        ...
        RayTraceMemory memoryx = new RayTraceMemory(Vec3.createVectorHelper(player.posX, player.posY + eyeHeight, player.posZ), Vec3.createVectorHelper((double)((float)x + posX), (double)((float)y + posY), (double)((float)z + posZ)), side);
