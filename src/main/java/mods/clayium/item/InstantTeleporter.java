...
public class InstantTeleporter extends ClayShooter {
    ...
    @Override
    public void spawnBullet(World worldIn, EntityPlayer playerIn, ItemStack stack, ClayShooter shooter, float per, boolean critical) {
        if (!playerIn.world.isRemote) {
            EntityTeleportBall teleportBall = new EntityTeleportBall(worldIn, playerIn, shooter.lifespan, shooter.initialVelocity, shooter.diffusion, shooter.bulletDamage, 1, critical);
            teleportBall.shoot(playerIn, playerIn.rotationPitch, playerIn.rotationYaw, 0.0F, shooter.initialVelocity, shooter.diffusion);
